...
class SignalView(QWidget):
    def __init__(self, play_pause_key=Qt.Key.Key_Space, move_left_key=Qt.Key.Key_Left, move_right_key=Qt.Key.Key_Right,
                 load_csv_key=Qt.Key.Key_Control + Qt.Key.Key_O, zoom_in_key=Qt.Key.Key_Control + Qt.Key.Key_Up,
                 zoom_out_key=Qt.Key.Key_Control + Qt.Key.Key_Down, parent=None):
    ...
    def initUI(self):
        ...
        # Container that holds the buttons so it can be hidden or shown
    ...
    def update(self):
        ...
        for i in range(len(self.loaded_signals)):
            ...
            self.plot_widget.plot(self.x_values[i], self.y_values[i], pen=color, name=f'Signal {i + 1}')
