...
# Move selected signal to another PlotWidget
    def move_signal(self, target_widget):
        if self.selected_signal:
            ...
            sent_signal = pg.PlotDataItem(x = self.selected_signal.getData()[0],y= self.selected_signal.getData()[1] , pen = self.selected_signal.opts['pen'], name = self.selected_signal.opts['name'], clickable = True, radius = 2)
...
# Add signals
    def add_signal(self):
        ...
        signal = pg.PlotDataItem(data, pen = curve_color, name = "Signal_"+ str(len(self.loaded_signals)) ,clickable=True , radius = 2)
