...
class SignalView(QWidget):
    ...
    def initUI(self):
        ...
        self.load_button.setShortcut(load_csv_key)
    ...
    def update(self):
        if self.loaded_signals and self.current_index < len(self.loaded_signals[0]):
            for i in range(len(self.loaded_signals)):
                ...
                self.x_values[i].append(self.current_index)  # Use the index as a simple time placeholder
            ...
            for i in range(len(self.loaded_signals)):
                ...
                self.plot_widget.plot(self.x_values[i], self.y_values[i])
    ...
    def add_new_signal(self):
        ...
        # Append the data to loaded signals list
        self.loaded_signals.append(signal_data)
...
def main():
    ...
    signal_view1 = SignalView(QtCore.Qt.Key.Key_Space, QtCore.Qt.Key.Key_Left,QtCore.Qt.Key.Key_Right,QtCore.Qt.Key.Key_Control + QtCore.Qt.Key.Key_O,QtCore.Qt.Key.Key_Control + QtCore.Qt.Key.Key_Up,QtCore.Qt.Key.Key_Control + QtCore.Qt.Key.Key_Down)
    # signal_view2 = SignalView(QtCore.Qt.Key.Key_Space, QtCore.Qt.Key.Key_Left,QtCore.Qt.Key.Key_Right,QtCore.Qt.Key.Key_Control + QtCore.Qt.Key.Key_O,QtCore.Qt.Key.Key_Control + QtCore.Qt.Key.Key_Up,QtCore.Qt.Key.Key_Control + QtCore.Qt.Key.Key_Down)
