...
class pltDataItem(pg.PlotDataItem):
    ...
    def mousePressEvent(self,event):
        if event.button() == QtCore.Qt.RightButton:
            ...
            print(f"Left button clicked {self.name}")
...
# Appends a pltDataItem to loaded_signals
    def add_to_loaded_signals(self, new_signal = pltDataItem):
...
# Move selected signal to another PlotWidget
    def move_signal(self, target_widget):
        if self.selected_signal:
            ...
            sent_signal = pltDataItem(x = self.selected_signal.getData()[0],y= self.selected_signal.getData()[1] , pen = self.selected_signal.opts['pen'], name = self.selected_signal.opts['name'], clickable = True )
...
# Add signals
    def add_signal(self):
        ...
        signal = pltDataItem(data, pen = curve_color, name = curve_color +"_Signal",clickable=True)
