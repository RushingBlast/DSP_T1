    ...
    def update(self):
        if self.loaded_signals:
            ...
            for curve in self.loaded_signals:
                ...
                # TODO - IF this doesn't reach anything remove it
                # if self.current_index > len(curve.original_data):
                #     curve.setData(curve.original_data)    
...
# Move selected signal to another PlotWidget
    def move_signal(self, target_widget):
        if self.selected_signal:
            ...
            # Add the Clone to the target widget and setk range automatically
