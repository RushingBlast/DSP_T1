...
class SignalView(QWidget):
    ...
    def update_new(self):
        if len(self.loaded_signals) != 0 :
            ...
            # self.plot_widget.clear()
            coefficient = 1
            # Display a portion of the loaded signals
            for signal in self.loaded_signals:
                #Get the data within the specified range
                subset_data = signal.getData()[1][self.current_index *coefficient : (self.current_index + 1) * coefficient]
                curve_subset = pg.PlotDataItem(subset_data, pen = signal.opts['pen'])
                self.plot_widget.addItem(curve_subset)
                self.current_index += 1
                # self.plot_widget.setXRange(self.current_index *coefficient , (self.current_index + 1) * coefficient*)
