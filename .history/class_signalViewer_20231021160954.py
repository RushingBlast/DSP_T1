...
"""CUSTOM DIALOG BOX TO RENAME SIGNALS"""
class dialogRename(QDialog):
    def __init__(self, parent = None):
        self.setWindowTitle("Enter new signal name")
        dialog_buttons = QtWidgets.QDialogButtonBox.Ok | QtWidgets.QDialogButtonBox.Cancel
        
        self.buttonBox = QtWidgets.QDialogButtonBox(dialog_buttons)
        self.buttonBox.accepted.connect(self.return_new_name)
        self.buttonBox.rejected.connect(self.reject)
        self.layout = QtWidgets.QVBoxLayout()
        
        lineEdit_rename = QtWidgets.QLineEdit(self) # Takes input from user
        self.layout.addWidget(lineEdit_rename)
        self.layout.addWidget(self.buttonBox)
        self.setLayout(self.layout)
    
    def return_new_name(self):
        return self.lineEdit_rename.text()
...
################################################      CLASS VARIABLES      ################################################
        ...
        self.is_signal_edit_buttons_enabled = False # Flag to ease toggilng of signal selection-dependent buttons
        ...
        self.toggle_signal_buttons() # Disable signal-dependendent-buttons on run
...
# HANDLE CUSTOM CONTEXT MENU ON RIGHT CLICKING PLOT ITEM
        # self.view_widget.customContextMenuRequested.connect(self.handleContextMenu)
...
# Hande Context Menu
    def handleContextMenu(self, pos):
        if self.view_widget.getPlotItem().sceneBoundingRect().contains(pos):
            ...
            self.view_widget.showMenu(pos)

# Custom context menu
    def signal_context_menu(self, pos):
            custom_menu = QtWidgets.QMenu(self.view_widget)
...
# Rename selected signal
    def rename_selected_signal(self):
        dialog = dialogRename(self)
        new_name = dialog.return_new_name()
        dialog.exec()
        


#TODO - This was an experiment and failed. Either fix or remove
...
# Return Clicked signal
    def return_clicked_signal(self, event):
        self.toggle_signal_buttons()
...
# Function to enable/disable buttons that need a signal to be selected
    def toggle_signal_buttons(self):
        self.is_signal_edit_buttons_enabled = not self.is_signal_edit_buttons_enabled
        self.btn_change_color.setEnabled(self.is_signal_edit_buttons_enabled)
        self.btn_transfer.setEnabled(self.is_signal_edit_buttons_enabled)
        self.btn_remove.setEnabled(self.is_signal_edit_buttons_enabled)
