...
# Return Clicked signal
    def return_clicked_signal(self):
        ...
        print(f"Index of selected signal is: {self.loaded_signals.index(self.sender())}")
...
# Update plot widget
    ...
    def move_signal(self, target_widget):
        if self.selected_signal:
            ...
            sent_signal.sigClicked.connect(self.return_clicked_signal)
            sent_signal.color = self.selected_signal.opts['pen']
            sent_signal.original_data = self.selected_signal.original_data
            target_widget.loaded_signals.append(sent_signal)
            
            # Remove the signal from the owner widget (self)
            self.loaded_signals.remove(self.selected_signal)
            self.view_widget.removeItem(self.selected_signal)
            ...
            self.selected_signal = any # Empty the selected_signal variable
