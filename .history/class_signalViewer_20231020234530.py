...
import numpy as np
import sys
from pyqtgraph import PlotWidget, plot, pltDataItem
...
class pltDataItem(pg.pltDataItem):
    # Mouse click modifier
    def mousePressEvent(self,event):
        if event.button() == QtCore.Qt.RightButton:
            print(f"Right button clicked {self.name}")
...
# Handles right clicking
    def handle_right_click(self, pltDataItem):
        print(f"right clicked signal {pltDataItem.name}")
...
# Appends a pltDataItem to loaded_signals
    def add_to_loaded_signals(self, new_signal = pg.pltDataItem):
...
# Return Clicked signal
    def return_clicked_signal(self, event):
        if event.button() == QtCore.Qt.LeftButton:
            ...
            self.signal_context_menu(event.pos)            
...
# Move selected signal to another PlotWidget
    def move_signal(self, target_widget):
        if self.selected_signal:
            ...
            sent_signal = pg.pltDataItem(x = self.selected_signal.getData()[0],y= self.selected_signal.getData()[1] , pen = self.selected_signal.opts['pen'], name = self.selected_signal.opts['name'], clickable = True )
...
# Add signals
    def add_signal(self):
        ...
        signal = pg.pltDataItem(data, pen = curve_color, name = curve_color +"_Signal",clickable=True)
